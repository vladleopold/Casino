...
import { getVipRouteContent } from "@slotcity/cms-sdk";
...
export default async function VipPage() {
  const routeContent = await getVipRouteContent();

  return (
    <LayoutShell route="vip">
      <main className="slotcity-home slotcity-route-page">
        ...
        <section className="slotcity-hero-grid slotcity-route-hero-grid">
          <Reveal className="slotcity-hero-card slotcity-hero-card-primary" data-block-id="vip-hero">
        ...
        <Reveal className="slotcity-section" delay={0.1}>
          <div className="slotcity-section-head">
            <h2>VIP добірка слотів</h2>
            <TrackedLink href={"/catalog" as Route} className="slotcity-inline-link" event="cta_clicked">
              Усі
            </TrackedLink>
          </div>
          <div className="slotcity-game-grid">
            {routeContent.vipGames.map((game, index) => (
              <TrackedGameCard key={game.id} title={game.title} provider={game.provider} image={game.image} gameId={game.id} shelfId="vip_slots" position={index + 1} />
            ))}
          </div>
        </Reveal>
        ...
        <Reveal className="slotcity-section" delay={0.12}>
          <div className="slotcity-section-head">
            <h2>VIP live-зали</h2>
            <TrackedLink href={"/live" as Route} className="slotcity-inline-link" event="cta_clicked">
              Усі
            </TrackedLink>
          </div>
          <div className="slotcity-live-grid">
            {routeContent.loungeGames.map((game, index) => (
              <TrackedGameCard key={game.id} title={game.title} provider={game.provider} image={game.image} gameId={game.id} shelfId="vip_live" position={index + 1} />
            ))}
          </div>
        </Reveal>

        <Reveal className="slotcity-editorial-grid" delay={0.14}>
          {routeContent.benefits.map((item) => (
